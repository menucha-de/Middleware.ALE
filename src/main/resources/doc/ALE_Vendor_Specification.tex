...
\fancyfootoffset{5mm}
\fancyfoot{}
\lfoot{\footnotesize \it{Copyright \textcopyright  \textcolor{blue}{Menucha Team.} All Rights Reserved.}}
\cfoot{}
...
\ifpdf
\includegraphics[width=400px]{img/logo.png}
\else
\begin{img}
\input{img/logo.txt}
\end{img}
...
\bigskip
{\color{blue}
\copyright Menucha Team,}\ifpdf\\\fi
...
means of electronic systems without the written permission of Menucha Team
...
This document specifies vendor extensions and implementation specific behavior of the ALE Menucha Team Vendor implementation of the ALE 1.1.1 standard specification. 

\section{ALE Vendor Interfaces}
The Menucha Team vendor specification for ALE defines two additional interfaces, as defined below.
...
Each of the two Menucha Team Vendor APIs includes a pair of methods having the following signature:
...
This section defines the implementation depended behavior for fieldnames that are pre-defined by the ALE specification. The Menucha Team  vendor implementation recognizes each fieldname defined in section 6.1 of the ALE specification document and interprets it as defined in the specification. In addition, this ALE implementation implements the Tag Memory API and recognizes fieldnames defined through that API (see section 7 of the ALE specification document).

\subsubsection{Type of Tags}
When interacting with a Gen2 tag, the Menucha Team  vendor implementation behaves as specified in the ALE specification document section 6.1. When interacting with any other type of tag, this ALE implementation will not consider that tag and will exclude any tag types others then Gen2 from all reports.

\subsubsection{Memory Bank Fieldnames}
The Menucha Team  vendor implementation recognizes any memory bank fieldnames that are specified in section 6.1 of the ALE specification document.\\
...
This Menucha Team  vendor implementation recognizes any string specified in sub-section 6.1.9.2 of the ALE specification document as a valid variable fieldname. This ALE implementation does not support those variable fieldnames for WRITE, READ, ADD and DELETE operations of the Writing API and for the Reading API, therefore an attempt to do so will always raise an ''operation not possible'' condition.

\subsection{Build-in Datatypes and Formats}
This section defines the implementation depended behavior for datatypes and formats that are pre-defined by the ALE specification. The Menucha Team vendor implementation recognizes each datatype and format defined in section 6.2 of the ALE specification document and interprets it as defined in the specification. 

\subsubsection{The \texttt{bits} Datatype}
The Menucha Team  vendor implementation recognizes the string bits as a valid datatype as specified in section 6.2.3 of the ALE specification document.
...
\begin{code}
\emph{Note that the  Menucha Team  vendor implementation supports \texttt{primaryKeyFields} for all logical readers.This ALE implementation will therefore not raise an \texttt{ECSpecValidationException} if the \texttt{primaryKeyFields} parameter is specified.}
\end{code}
...
The Menucha Team  vendor implementation provides support for real-time clock trigger URIs as defined in section 8.2.4.1 of the ALE specification document.
...
URIs beginning with the string \texttt{urn:havis:ale:trigger:http:} are reserved for triggers specified below. The Menucha Team  vendor implementation provides support for trigger URIs of this form. This ALE implementation conforms to the following specification for all such URIs valid according to the specification below.
...
URIs beginning with the string \texttt{urn:havis:ale:trigger:port:} are reserved for triggers specified below. The Menucha Team vendor implementation provides support for trigger URIs of this form. This ALE implementation conforms to the following specification for all such URIs valid according to the specification below.
...
The Menucha Team vendor implementation interprets an instance of  \texttt{ECReportSetSpec} as specified in section 8.2.6 of the ALE specification document.
...
The Menucha Team vendor implementation adds one \texttt{ECReaderStat} for each reader to the \texttt{statBlocks} parameter list of an \texttt{ECTagStat} and the \texttt{ECTagStat} is included in an 
\ifpdf\\ \fi
...
\texttt{ECTagCountStat} is a subclass of \texttt{ECTagStat}. The Menucha Team vendor implementation includes one \\\texttt{ECTagCountStat} in an \texttt{ECReportGroupListMember} if the \texttt{TagCount} statistics profile was included in the corresponding \texttt{ECReportSpec}.\\\texttt{ECTagCountStat} includes all of the fields in \texttt{ECTagStat}, plus the following additional fields:
...
The Menucha Team vendor implementation recognizes the values \texttt{epcBank} and \texttt{userBank} as valid fieldspec values for the CHECK \texttt{CCOpType} as defined in section 9.3.5.1 of the ALE specification document. 
...
The Menucha Team vendor implementation recognizes the values \texttt{epcBank} and \texttt{userBank} as valid fieldspec values for the INITIALIZE \texttt{CCOpType} as defined in section 9.3.5.2 of the ALE specification document. 
...
This Menucha Team vendor implementation interprets an RNGSpec as specified in section 9.7.2 of the ALE specification document. This ALE implementation does not define extensions to that class in order to provide additional parameters to control the behavior of a random number generator.
\fi
...
The Menucha Team vendor implementation recognizes a set of \texttt{LRProperty} parameters, which an ALE client set using the \texttt{properties} parameter of an \texttt{LRSpec} or the \texttt{setProperties} method of the Logical Reader API for the common namespace. This ALE implementation interprets these parameters as follows:
...
The Menucha Team vendor implementation recognizes a set of \texttt{LRProperty} parameters, which an ALE client set using the properties parameter of an \texttt{LRSpec} or the \texttt{setProperties} method of the Logical Reader API for the controller namespace. This ALE implementation interprets these parameters as follows:
...
The Menucha Team vendor implementation provides support for all physical readers of the type HARTING RF-R. The ALE implementation provides connector implementations to handle the communication between the ALE and all supported RF-R readers. These connector types can be initialized according to the following table, using the properties parameter of an \texttt{LRSpec} or the \texttt{setProperties} method of the Logical Reader API.
...
The Menucha Team vendor implementation provides support for the built-in reader of the RFID Reader RF-R350. The ALE implementation provides a connector implementation to handle the communication between the ALE and the built-in reader. The ALE automatically detects the built-in reader and predefines the necessary \texttt{LRSpec} with the name \texttt{BuiltIn}. Both \texttt{getLogicalReaderNames} and \texttt{getLRSpec} will include the built-in reader.

The Built-In Connector currently supports no \texttt{LRProperty} parameters.


\subsubsection{Reader Namespace}
The Menucha Team vendor implementation recognizes a set of \texttt{LRProperty} parameters, which an ALE client set using the \texttt{properties} parameter of an \texttt{LRSpec} or the \texttt{setProperties} method of the Logical Reader API for the reader namespace.
...
The Menucha Team vendor implementation does fully support tag smoothing as specified in section 10.6 of the ALE specification document for EC\ifincludecc , CC \fi and the PC API. Therefore this ALE implementation will not raise a \texttt{ValidationException} when a client sets the tag smoothing properties.
...
The Menucha Team vendor implementation does not support the ALE Access Control API specified in section 11 of the ALE specification document.
...
The Menucha Team vendor implementation delivers event cycle, command cycle or port cycle reports by sending an HTTP POST request to the callback receiver designated in the URI, where  \texttt{remainder-of-URL} is included in the HTTP \texttt{request-line}, and where the payload is the \texttt{ECReports} instance\ifincludecc , \texttt{CCReports} instance \fi or \texttt{PCReports} instance encoded in XML according to the schema.
...
The Menucha Team vendor implementation delivers event cycle, command cycle or port cycle reports by opening a new TCP connection to the specified host and port, writing to the connection the \texttt{ECREports} instance\ifincludecc , \texttt{CCReports} instance \fi or \texttt{PCReports} instance, encoded in XML according to the schema and then closing the connection. The implementation does not require a reply or acknowledgement.
\fi
...
The Menucha Team vendor implementation delivers event cycle, command cycle or port cycle reports by directly writing the \texttt{ECREports} instance\ifincludecc , \texttt{CCReports} instance \fi or \texttt{PCReports} instance, encoded in XML according to the schema, to the UDP socket. The implementation does not require a reply or acknowledgement.
\fi
...
The Menucha Team vendor implementation delivers event cycle, command cycle or port cycle reports by sending one or more INSERT operations to the specified table and columns within the database, and where the values are the \texttt{ECReports} instance\ifincludecc , \texttt{CCReports} instance \fi or \texttt{PCReports} instance information mapped on columns as specified through the URI see section \ref{subsec: SQLB} for mapping options.
...
\paragraph{ECReports}
The Menucha Team vendor implementation provides SQL binding mapping parameters for \texttt{ECReports} as specified in the below table. 
...
\ifincludecc
\paragraph{CCReports}
The Menucha Team vendor implementation provides SQL binding mapping parameters for \texttt{CCReports} as specified in the below table. 
...
\paragraph{PCReports}
The Menucha Team vendor implementation provides SQL binding mapping parameters for \texttt{PCReports} as specified in the below table. 
...
The Menucha Team vendor implementation delivers event cycle\ifincludecc , command cycle \fi or port cycle reports by executing a MQTT publish command to the specified MQTT Broker and topic, while the message will be encoded in JSON format.
...
The Menucha Team vendor implementation provides the following ALE web service URIs:
...
The Menucha Team vendor implementation implements the methods of the ALE Reader Configuration API as specified in the following table:
...
The Menucha Team vendor implementation constructs an RCConfiguration as follows:
...
The Menucha Team vendor implementation constructs an RCStyleSheet as follows:
...
The Menucha Team vendor implementation constructs an RCNode as follows:
...
The Menucha Team vendor implementation constructs an RCProperty as follows:
